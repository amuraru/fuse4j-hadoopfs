...
public class FuseHdfsClient implements Filesystem3, XattrSupport, LifecycleSupport {
  ...
  public int getattr(String path, FuseGetattrSetter getattrSetter)
    ...
    FuseContext ctx = FuseContext.get();
    HdfsFileAttr s = hdfs.getFileInfo(ctx.uid, path);
    ...
    getattrSetter.set(s.inode, s.getMode(), s.numberOfLinks, s.uid, s.gid, 0, size, blocks, s.accessTime,
                      s.modifiedTime, s.createTime);
  ...
  public int getdir(String path, FuseDirFiller filler)
    ...
    HdfsDirEntry[] entries = hdfs.listPaths(FuseContext.get().uid, path);
  ...
  public int mknod(String path, int mode, int rdev)
    ...
    return hdfs.mknod(FuseContext.get().uid, path) ? 0 : FuseException.EREMOTEIO;
  ...
  public int mkdir(String path, int mode)
    ...
    boolean status = hdfs.mkdir(FuseContext.get().uid, path);
  ...
  public int unlink(String path)
    ...
    boolean status = hdfs.unlink(FuseContext.get().uid, path);
  ...
  public int rmdir(String path)
    ...
    boolean status = hdfs.rmdir(FuseContext.get().uid, path);
  ...
  public int rename(String from, String to)
    ...
    boolean status = hdfs.rename(FuseContext.get().uid, from, to);
  ...
  public int chown(String path, int uid, int gid)
    throws FuseException {
    log.info("chown(): " + path + "\n");
    throw new FuseException("chown not supported").initErrno(FuseException.ENOSYS);
  }
  ...
  public int truncate(String path, long size)
    ...
    mknod(path, 0, 0);
    Object fh = hdfs.open(FuseContext.get().uid, path, NativeIO.O_WRONLY | NativeIO.O_CREAT);
    if (fh == null) {
      return FuseException.EREMOTEIO;
    }
    if (!hdfs.close(FuseContext.get().uid, fh)) {
      return FuseException.EREMOTEIO;
    }
  ...
  public int utime(String path, int atime, int mtime)
    ...
    return hdfs.utime(FuseContext.get().uid, path, atime, mtime) ? 0 : FuseException.EACCES;
  }

  public int statfs(FuseStatfsSetter statfsSetter)
    throws FuseException {
    log.info("statfs(): \n");
    FuseStatfs status = hdfs.getStatus(FuseContext.get().uid);
    if (status == null)
      return FuseException.EREMOTEIO;
  ...
  public int open(String path, int flags, FuseOpenSetter openSetter)
    ...
    Object fh = hdfs.open(FuseContext.get().uid, path, flags);
    if (fh == null)
      return FuseException.EACCES;
    openSetter.setFh(fh);
    return 0;
  ...
  public int read(String path, Object fh, ByteBuffer buf, long offset)
    ...
    //return Errno.EBADF;
    log.info("read(): " + path + " offset: " + offset + " len: " + buf.capacity() + "FH:" + fh + "\n");

    if (fh == null || !hdfs.read(FuseContext.get().uid, fh, buf, offset))
      return FuseException.EREMOTEIO;
  ...
  public int write(String path, Object fh, boolean isWritepage, ByteBuffer buf, long offset)
    ...
    if (fh == null || !hdfs.write(FuseContext.get().uid, fh, buf, offset))
      return FuseException.EREMOTEIO;
  ...
  public int release(String path, Object fh, int flags)
    ...
    if (fh == null || !hdfs.close(FuseContext.get().uid, fh))
      return FuseException.EREMOTEIO;
    return 0;
  }
  ...
  public int flush(String path, Object fh)
    ...
    if (fh == null || !hdfs.flush(FuseContext.get().uid, fh))
      return FuseException.EREMOTEIO;
    return 0;
  }
